		  ...
		  <div class="col-lg-4" style="margin-top: 10px" ng-repeat="prod in prods.label">  
		    <div class="input-group">
		      <span class="input-group-addon" style="font-size: 16px" ng-style="prods.style[$index]">
		        <input type="checkbox" ng-model="prods.enabled[$index]" ng-click="AutoSelect($index)"> สถานะของ<%prod%>
		      </span>
  	...
  	<div class="container col-md-12" style="margin-top: 20px">
		  ...
		  <div class="col-lg-3" style="margin-top: 10px" ng-repeat="prod in prods.label">  
		    <div class="input-group">
		      ...
		      <select class="form-control" ng-options = "idx as item for (idx, item) in medias.label" ng-model="medias.model[$index]" ng-show="medias.enabled[$index]" style="height: 32px">
    		  </select>
		    </div><!-- /input-group -->
		  </div><!-- /.col-lg-3 -->
  	</div> <!-- End Media container -->
    ...
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">  
          <table class="table table-striped" ng-show="book_table">
          ...
          <table class="table table-striped" ng-show="!book_table">
            <tr style="font-size: 18px;color: #4d4d4d">
              ...
              <tr ng-repeat="(i,b_detail) in report.media.braille_detail">
                <td><%FindBook(b_detail.braille_id, 'braille')['book'].title%></td>
                <td>เบรลล์</td>
              ...
              <tr ng-repeat="(i,b_detail) in report.media.cassette_detail">
                <td><%FindBook(b_detail.cassette_id, 'cassette')['book'].title%></td>
                <td>คาสเช็ท</td>
              ...
              <tr ng-repeat="(i,b_detail) in report.media.daisy_detail">
                <td><%FindBook(b_detail.daisy_id, 'daisy')['book'].title%></td>
                <td>เดซี่</td>
              ...
              <tr ng-repeat="(i,b_detail) in report.media.cd_detail">
                <td><%FindBook(b_detail.cd_id, 'cd')['book'].title%></td>
                <td>ซีดี</td>
                <td><%b_detail.cd_id%></td>
                <td><%b_detail.id%></td>
                <td><%b_detail.part%> / <%FindBook(b_detail.cd_id, 'cd')['media'].numpart%></td>
                <td><%DetailStatus(b_detail.status)%></td>     
              </tr>
              <tr ng-repeat="(i,b_detail) in report.media.dvd_detail">
                <td><%FindBook(b_detail.dvd_id, 'dvd')['book'].title%></td>
                <td>ดีวีดี</td>
                <td><%b_detail.dvd_id%></td>
                <td><%b_detail.id%></td>
                <td><%b_detail.part%> / <%FindBook(b_detail.dvd_id, 'dvd')['media'].numpart%></td>
                <td><%DetailStatus(b_detail.status)%></td>     
              </tr>
          </table>
        </div>
