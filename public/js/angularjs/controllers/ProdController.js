...
app.controller('ModalInstanceCtrl', function ($rootScope, $scope, $uibModalInstance, media_type, status_request, BookService, media, status, prods) {
	...
	status_request.$promise.then(function(response) {
	...
	$scope.getStatusLabel = function(num) {
		if(typeof num != "undefined")
			return status[media_type][num];
		return status[media_type][$scope.selected.status.id];
	}
	...
	$scope.getFormData = function() {
		return {
			book_id: $rootScope.selected_book,
			act_date: $rootScope.DBDate($scope.action_date),
			finish_date: $rootScope.DBDate($scope.finish_date),
			media_type: media_type,
			action: $scope.selected.status.id,
			actioner: $scope.actioner
		}
	}
	...
	$scope.ok = function () {
		if($scope.validateForm()) {
			...
			BookService.createProdStatus(params, function(response) {
				$uibModalInstance.close();
				prods($scope.getFormData());
			},
			function(error) {
